...
var SpiffyBird = (function() {
	...
	function populateMediaListFromJSPF(jspf, mediaList, library){
		mediaList.clear();
		...
		for (var i = 0, l = jspf.playlist.track.length; i < l; i++) {
			...
			try {
				...
				if (!mediaItem){ // didn't find it in the library or didn't have an identifier to search for
					...
					var mediaItem = library.createMediaItem(track.location[0] || 'http://grabb.it/music/'+track.creator+'/'+track.title+'.mp3');
				}
	...
	function buildMediaListFromJSPF(jspf, library){
		var mediaList = library.createSimpleMediaList(jspf.playlist["title"]);
		mediaList.clear(); // syncing

		return populateMediaListFromJSPF(jspf, mediaList, library);
	};
	...
	return {
		...
		utils : {
			...
			JSPFinto : function(jspf, mediaList, library){
				return populateMediaListFromJSPF(jspf, mediaList, library); 
			}
		},
		...
		localPlaylist : {	
			save : function(jspf) {
				...
				buildMediaListFromJSPF(jspf, library); // since we use the main library this actually installs the playlist
			},
			get : function(query) {
				var results = []
